...
- **Factored dir:** `factored_out_05b_r1.5` (factored)
...
|---|---|---|---|
| Mean tok/s | 64.4 | 63.9 | 64.4 tok/s → 63.9 tok/s (-0.8% ~) |
| Median tok/s | 64.8 | 64.2 | 64.8 tok/s → 64.2 tok/s (-1.0% ~) |
| Mean TTFT | 1 ms | 1 ms | 1 ms → 1 ms (+3.1% ~) |
| Total bench time | 18.4 s | 18.0 s | 18.4 s → 18.0 s (-2.0% ✓) |
| Model load | 1.5 s | 3.7 s | 1.5 s → 3.7 s (+151.9% ✗) |
...
| RSS peak | 2883.1 MB | 3488.1 MB | 2883.1 MB → 3488.1 MB (+21.0% ✗) |
| RSS mean | 2856.5 MB | 3461.4 MB | 2856.5 MB → 3461.4 MB (+21.2% ✗) |
| CPU peak % | 128.1% | 198.4% | 128.1% → 198.4% (+54.9% ✗) |
| CPU mean % | 99.1% | 100.0% | 99.1% → 100.0% (+0.9% ~) |
...
| `fact_year` | factual | 80 | 59 | 74% |
| `comp_fox` | completion | 47 | 29 | 62% |
| `comp_road` | completion | 80 | 17 | 21% |
| `comp_storm` | completion | 80 | 10 | 12% |
| `reason_syllogism` | reasoning | 80 | 13 | 16% |
...
| `summary_photosynthesis` | summary | 80 | 19 | 24% |
| `summary_gravity` | summary | 80 | 52 | 65% |

**Overall greedy agreement:** 560/1119 = **50.0%**

By prompt type:

- **code**: 141/141 = 100.0%
- **completion**: 56/207 = 27.1%
- **factual**: 250/371 = 67.4%
- **reasoning**: 42/240 = 17.5%
- **summary**: 71/160 = 44.4%
...
| `fact_math` | factual | 51 | 51 | 100% |
| `fact_year` | factual | 80 | 61 | 76% |
| `comp_fox` | completion | 47 | 30 | 64% |
| `comp_road` | completion | 80 | 25 | 31% |
| `comp_storm` | completion | 80 | 21 | 26% |
| `reason_syllogism` | reasoning | 80 | 18 | 22% |
| `reason_inverse` | reasoning | 80 | 22 | 28% |
| `reason_arithmetic` | reasoning | 80 | 18 | 22% |
| `code_factorial` | code | 80 | 80 | 100% |
| `code_fizzbuzz` | code | 61 | 61 | 100% |
| `summary_photosynthesis` | summary | 80 | 23 | 29% |
| `summary_gravity` | summary | 80 | 79 | 99% |

**Overall top-5 overlap:** 648/1119 = **57.9%**
...
**Factored:** ' I took the one less crowded one, And entered on a gleaming hall of gold. The door was shut and the doorkeeper said, "Come in, sir." I entered, and stood before the altar, And there was a gold candlestick before the altar, And a golden lampstand with a silver bowl of oil in the middle of it. I looked at the candlestick and said,'
...
**Factored:** " warm-blooded.  If the premise is true, what can we conclude about the hypothesis?  Hint: The body's second line of defense against pathogens includes the inflammatory response. If bacteria enter the skin through a scrape, the area may become red, warm, and painful. These are signs of inflammation. Inflammation is one way the body reacts to infections or injuries. Inflammation is"
...
**Factored:** ' plants, algae, and some bacteria convert light energy into chemical energy in the form of glucose. Photosynthesis is the process by which plants, algae, and some bacteria convert light energy into chemical energy in the form of glucose. Photosynthesis is the process by which plants, algae, and some bacteria convert light energy into chemical energy in the form of glucose. Photosynthesis is the pr'
